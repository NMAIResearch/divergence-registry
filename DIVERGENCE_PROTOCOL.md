# Divergence Protocol: subject in, checkable artefact out

**v0.2, 20 August 2026. On trial.** Self-contained: everything needed to run this is in this
document and the two scripts beside it. No other files and no shared filesystem are assumed.

**Version status.** The method is being tested by use, not treated as finished. No subject has
completed v0.2 end to end. Do not treat any part of this as settled until it has survived a real
subject.
...
Two competent researchers, same subject, same protocol, will **not** reach the same conclusion.
Anyone promising that is selling something. What this protocol enforces is weaker and more useful:
...
**Do not describe an artefact built this way as "reproducible research" without the qualifier.**
The reproducible part is the process record, plus any code that regenerates figures from data. The
...
**Attribution caution.** The last one is the subtle one and it is worth understanding before you skip it. A wrongly
...
work feels done. It is complete when its decisions are logged and its gate passes.** All 27
decision IDs below are required by `research_gate.py` (under the v0.2 contract), which refuses an artefact that skips one.
...
**Censoring rule. Where an entry can leave the population during the window, `S6` carries the
censoring treatment.** Name what counts as a confirmed exit and what is right-censored at the edge of the held data. An entry last
...
One counted 20 of 409 entries (4.89 per cent); the other counted 14 of 409 (3.42 per cent), N = 409.
The gap was not attributable to any decision row. Survival, churn, delisting, deprecation and
queue-drop questions all have this shape.
...
**Custody limit.** Neither proves authenticity: a hash of a scraped page proves you had that page, not that it was
...
**Filename fallback.** Some model harnesses refuse to write a file under that exact name. Where that happens, the
...
**A scope statement never substitutes a stated error rate.** "Roughly 5% inaccurate" has no numerator, no
...
**Run `--demo` before trusting a pass.** It builds complete worked projects on v0.2 and v0.1, asserts they pass,
then seeds faults into each arm and asserts the arms fire. A checker that has only ever returned
